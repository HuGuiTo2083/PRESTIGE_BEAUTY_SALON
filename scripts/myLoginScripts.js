...
if (myLogin) {
  ...
  myLogin.addEventListener('click', async () => {
    const myModal2 = document.getElementById('modalOverlay2')
    console.log('hola')
    const mySpinner = document.getElementById('mySpinner2')
    myModal2.style.display  = 'grid'
   ...
   mySpinner.style.display= 'none'
    ...
    if (response.status == 1) {
      ...
      if (params.has('myOrigin')) {                     
        
        setTimeout(() => {
          window.location.href = `./services.html?myId=${response.rows[0].usr_id}&myType=${response.rows[0].usr_type_user}`
        }, 2000)
